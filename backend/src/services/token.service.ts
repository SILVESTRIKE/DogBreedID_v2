...
import { PlainUser, userService } from "./user.service";
  ...
  async createTokens(
    ...
    const accessTokenOptions: SignOptions = {
      expiresIn: process.env
        .JWT_ACCESS_EXPIRATION as jwt.SignOptions["expiresIn"],
    };
    const refreshTokenOptions: SignOptions = {
      expiresIn: process.env
        .JWT_REFRESH_EXPIRATION as jwt.SignOptions["expiresIn"],
    };
    ...
    expiresAt.setSeconds(
      expiresAt.getSeconds() +
        parseInt(process.env.JWT_REFRESH_EXPIRATION_SECONDS!, 10)
    );
